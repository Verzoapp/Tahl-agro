...
const CreateActivity = () => {
    ...
    const {id} = useParams()
    const getActivity = activityList?.find(item => item?.id === id)
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-32 pb-10">
        <Link 
            href={`/dashboard/farm-activities/${id}`}
        ...
        <div className="mt-16">
          ...
          <p className="text-sm tracking-tight pt-1 text-gray-700">Fill out the information below to add <span className="text-[#2aa249]">{getActivity?.name}</span> of <span className="text-[#2aa249]">{getActivity?.cropProfile?.cropName}</span> to a farm lot</p> 
        </div>
          ...
          <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
            <form className=" flex h-auto w-[820px] flex-col" onSubmit={handleSubmit(onSubmitHandler)}>
                <div className="flex w-full justify-between mb-10">
                    <div className="flex h-[70px] w-full flex-col items-start">
                      <label
                        htmlFor="First Name"
